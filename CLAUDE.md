...
This is the monorepo root. Per-app conventions live in `apps/web/CLAUDE.md` and `apps/api/CLAUDE.md` and override anything here.
...
- `.claude-staging/` is gitignored. Use it for one-shot scratch only; never reference its files from the build.
...
If `next build` reports route conflicts ("two parallel pages resolve to the same path"), look for stub `page.tsx` files in route groups created by aborted refactors — see `apps/web/CLAUDE.md` § Routing.
