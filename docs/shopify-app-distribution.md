...
- [x] **Three GDPR / privacy webhooks.** `apps/api/src/server/webhooks/shopify-gdpr.ts` — POST handler at `/api/webhooks/shopify/gdpr/*` that verifies HMAC against `SHOPIFY_APP_API_SECRET` and dispatches on `x-shopify-topic`. Receipt event + dispatch outcome both audit-logged separately so reviewers see (a) the webhook arrived and verified, (b) what the redaction actually did:
  ...
  - `shop/redact` — calls `redactShop({ merchantId })` which hard-deletes 13 collections in dependency order (children before parents). Skipped with an audit row when a fresh install is in place at redact-time (the merchant uninstalled, then reconnected within the 48h Shopify retention window — the redact targets the prior install, not the live one). Belt-and-braces: also flips Shopify integrations to `disconnected` in case a parallel webhook delivery is racing.
