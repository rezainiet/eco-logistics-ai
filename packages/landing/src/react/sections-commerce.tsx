...
function ShopHeader({ values: v, env }: SectionProps) {
  ...
  const links = S.arr(v.links)
    .map((l) => ({ label: S.str(l.label), href: safeUrl(S.str(l.url)) }))
    .filter((l) => l.label && l.href);
  const nav = (className: string) =>
    links.length ? (
      <nav className={className}>
        {links.map((l, i) => (
          <a key={i} href={l.href!} className="inline-flex min-h-11 shrink-0 items-center whitespace-nowrap px-1 hover:text-[color:var(--lp-primary)]">
            {l.label}
          </a>
        ))}
      </nav>
    ...
    <header className={cx("w-full bg-[var(--lp-bg)]", minimal ? "border-b border-black/10" : "shadow-sm")}>
      <Container className="flex items-center justify-between gap-4 py-3">
        <div className="flex min-w-0 items-center gap-3">
          {logo ? <LandingImage value={logo} env={env} className="h-10 w-auto max-w-[140px] object-contain" placeholder="none" /> : null}
          {S.str(v.brandName) ? (
            <span className={cx("truncate", minimal ? cx("text-xl", TYPE.heading) : "text-xl font-bold text-[color:var(--lp-primary)]")}>
              {S.str(v.brandName)}
            </span>
          ) : null}
        </div>
        ...
        <div className="flex shrink-0 items-center gap-3">
          {phone && phoneHref && !minimal ? (
            <a href={phoneHref} className="hidden min-h-11 items-center gap-2 text-sm font-semibold lg:inline-flex">
              <Icon name="phone" className="h-4 w-4 text-[color:var(--lp-primary)]" />
              <span className="sr-only">{ctx.t.hotline}</span>
              {phone}
            </a>
          ) : null}
          <CtaButton value={S.cta(v.cta)} size="sm" variant={minimal ? "secondary" : "primary"} />
        </div>
      </Container>
...
function PromoHero({ values: v, env }: SectionProps) {
  ...
  const copy = (inverse: boolean) => (
    <div>
      {badge ? (
        <span className="mb-5 inline-flex items-center gap-2 rounded-full bg-[var(--lp-accent)] px-4 py-1.5 text-sm font-bold text-black/85">
          <Icon name="fire" className="h-4 w-4" />
          {badge}
        </span>
      ) : null}
      <h1 className={cx("text-4xl sm:text-5xl lg:text-6xl", TYPE.display)}>{S.str(v.headline)}</h1>
      {S.str(v.subheadline) ? (
        <p className={cx("mt-5 text-lg leading-[var(--lp-lh-body)] sm:text-xl", inverse ? "opacity-95" : "text-[color:var(--lp-muted)]")}>
          {S.str(v.subheadline)}
        </p>
      ) : null}
      ...
      {note ? (
        <p className={cx("mt-5 flex items-center gap-2 text-sm font-medium", inverse ? "opacity-90" : "text-[color:var(--lp-muted)]")}>
  ...
  if (banner) {
    ...
    return (
      <div className="relative isolate overflow-hidden bg-[var(--lp-primary)] text-white">
        {src ? (
    ...
    <div className="bg-[var(--lp-surface)]">
      <Container className="grid items-center gap-10 py-12 md:grid-cols-2 md:py-20">
        ...
        <div className="relative">
          <LandingImage value={img} env={env} icon="bag" className="aspect-[4/3] w-full rounded-[calc(var(--lp-radius)*1.5)] shadow-xl md:aspect-square" />
        </div>
      </Container>
    </div>
  );
    ...
    <div className="border-b border-black/10">
      <Container className="grid items-center gap-10 py-12 md:grid-cols-12 md:gap-12 md:py-20">
        <div className="md:col-span-5">
          {eyebrow ? <p className={cx("mb-6 text-[color:var(--lp-accent)]", TYPE.eyebrow)}>{eyebrow}</p> : null}
          <h1 className={cx("text-4xl sm:text-5xl lg:text-6xl", TYPE.display)}>{S.str(v.headline)}</h1>
          {S.str(v.subheadline) ? (
            <p className="mt-6 text-lg leading-[var(--lp-lh-body)] text-[color:var(--lp-muted)]">{S.str(v.subheadline)}</p>
          ) : null}
          <div className="mt-10">
          ...
          {items.map((c, i) => {
            ...
            const tile = (
              <div className="group relative overflow-hidden">
                <LandingImage value={S.img(c.image)} env={env} placeholder="gradient" icon="sparkles" className="aspect-[4/5] w-full" />
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-6 pt-16 text-white">
                  <span className={cx("text-2xl", TYPE.heading)}>{S.str(c.name)}</span>
                </div>
              </div>
            );
            return href ? (
        ...
        {items.map((c, i) => {
          ...
          const inner = (
            <span className="flex flex-col items-center gap-3 text-center">
              <span className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-full bg-[var(--lp-surface)] ring-1 ring-black/10 sm:h-24 sm:w-24">
                {img ? (
              ...
              <span className="text-sm font-semibold leading-[var(--lp-lh-snug)] sm:text-base">{S.str(c.name)}</span>
            </span>
          );
          return href ? (
  ...
  return (
    <div className={cx(!minimal && "bg-[var(--lp-surface)]")}>
      <Container className="py-14 md:py-20">
        <SectionHeading title={S.str(v.heading)} intro={S.str(v.intro)} align={minimal ? "left" : "center"} />
        <div
          className={cx(
        ...
        >
          {S.arr(v.items).map((p, i) => (
            <ProductCard key={i} product={p} ctx={ctx} variant={minimal ? "minimal" : "cards"} />
          ))}
        </div>
        <div className="mt-10 flex justify-center">
          <CtaButton value={S.cta(v.viewAll)} variant="secondary" requireLink />
        </div>
      </Container>
    </div>
  );
...
function OfferBanner({ values: v, env }: SectionProps) {
    ...
    return (
      <Container className="py-16 md:py-20">
        <div className="grid items-stretch overflow-hidden bg-[var(--lp-surface)] ring-1 ring-black/10 md:grid-cols-2">
          <LandingImage value={S.img(v.image)} env={env} placeholder="gradient" icon="sparkles" className="aspect-[4/3] w-full md:aspect-auto md:h-full" />
          <div className="flex flex-col justify-center p-8 md:p-14">
            {S.str(v.badge) ? <p className={cx("mb-4 text-[color:var(--lp-accent)]", TYPE.eyebrow)}>{S.str(v.badge)}</p> : null}
            ...
            <div className="mt-6">
              <PriceRow price={price} oldPrice={oldPrice} ctx={ctx} size="lg" />
            </div>
            {deadline ? (
              <p className="mt-3 flex items-center gap-2 text-sm text-[color:var(--lp-muted)]">
  ...
  return (
    <Container className="py-12 md:py-16">
      <div className="grid items-center gap-8 overflow-hidden rounded-[calc(var(--lp-radius)*1.5)] bg-[var(--lp-primary)] p-6 text-[color:var(--lp-on-primary)] sm:p-10 md:grid-cols-2">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            {S.str(v.badge) ? (
              <span className="inline-flex items-center gap-1.5 rounded-full bg-[var(--lp-accent)] px-3 py-1 text-sm font-bold text-black/85">
          ...
          <h2 className={cx("mt-5 text-3xl sm:text-4xl", TYPE.heading)}>{S.str(v.heading)}</h2>
          {S.str(v.text) ? <p className="mt-3 text-lg leading-[var(--lp-lh-body)] opacity-95">{S.str(v.text)}</p> : null}
          <div className="mt-6">
            <PriceRow price={price} oldPrice={oldPrice} ctx={ctx} size="lg" inverse />
          </div>
          {deadline ? (
            <p className="mt-3 flex items-center gap-2 text-sm font-medium opacity-90">
              <Icon name="clock" className="h-4 w-4 shrink-0" />
              {deadline}
            </p>
          ) : null}
        ...
        <LandingImage value={S.img(v.image)} env={env} placeholder="soft" icon="bag" className="aspect-[4/3] w-full rounded-[var(--lp-radius)] md:aspect-square" />
      </div>
    </Container>
  );
}

function TrustFeatures({ values: v }: SectionProps) {
  ...
  return (
    <div className={cx(cards ? "" : "border-y border-black/5 bg-[var(--lp-bg)]")}>
      <Container className={cards ? "py-16 md:py-20" : "py-10 md:py-12"}>
        {S.str(v.heading) ? <SectionHeading title={S.str(v.heading)} /> : null}
          ...
          {items.map((it, i) => (
            ...
            >
              ...
              <span className="min-w-0">
                <span className="block font-semibold leading-[var(--lp-lh-snug)]">{S.str(it.title)}</span>
                {S.str(it.text) ? (
                  <span className="mt-1 block text-sm leading-[var(--lp-lh-snug)] text-[color:var(--lp-muted)]">{S.str(it.text)}</span>
                ) : null}
              </span>
            </div>
          ))}
    ...
    <Container className="py-16 md:py-20">
      <div className="grid gap-10 lg:grid-cols-5">
        <div className="lg:col-span-2">
          ...
          {S.str(v.intro) ? <p className="mt-4 text-lg leading-[var(--lp-lh-body)] text-[color:var(--lp-muted)]">{S.str(v.intro)}</p> : null}
          {payments.length ? (
            <div className="mt-8">
              <p className="mb-3 font-semibold">{S.str(v.paymentHeading)}</p>
        ...
        <div className="lg:col-span-3">
          <div className="overflow-hidden rounded-[var(--lp-radius)] ring-1 ring-black/10">
            {zones.map((z, i) => {
              ...
              return (
                <div key={i} className="grid grid-cols-[1fr_auto] items-center gap-x-4 gap-y-1 border-b border-black/5 bg-[var(--lp-bg)] p-4 last:border-b-0 sm:grid-cols-[1.4fr_1fr_auto] sm:p-5">
                  <span className="flex items-center gap-2 font-semibold">
                    <Icon name="map-pin" className="h-4 w-4 shrink-0 text-[color:var(--lp-primary)]" />
                    {S.str(z.area)}
                  </span>
                  <span className="col-start-1 row-start-2 flex items-center gap-2 text-sm text-[color:var(--lp-muted)] sm:col-start-2 sm:row-start-1">
                    <Icon name="clock" className="h-4 w-4 shrink-0" />
                    {S.str(z.time)}
                  </span>
                  <span className="row-span-2 whitespace-nowrap text-right font-bold text-[color:var(--lp-primary)] sm:row-span-1">
          ...
          {S.str(v.note) ? <RichText value={S.str(v.note)} className="mt-4 text-sm text-[color:var(--lp-muted)]" /> : null}
        </div>
      </div>
    </Container>
  );
  ...
  return (
    <footer className="bg-[var(--lp-text)] text-[color:var(--lp-bg)]">
      <Container className="grid gap-10 py-14 sm:grid-cols-2 lg:grid-cols-4">
        <div className="lg:col-span-2">
          <p className={cx("text-2xl", TYPE.heading)}>{S.str(v.brandName)}</p>
          {S.str(v.tagline) ? <p className="mt-2 max-w-md leading-[var(--lp-lh-body)] opacity-80">{S.str(v.tagline)}</p> : null}
          {payments.length ? (
        ...
        <div className="space-y-2 text-sm">
          ...
          {phone ? (
            <a href={ctaHref({ kind: "phone", phone })?.href ?? undefined} className="flex min-h-11 items-center gap-2 hover:underline">
              <Icon name="phone" className="h-4 w-4 shrink-0" />
              {phone}
            </a>
          ) : null}
          {email ? (
            <a href={ctaHref({ kind: "email", email })?.href ?? undefined} className="flex min-h-11 items-center gap-2 break-all hover:underline">
              <Icon name="chat" className="h-4 w-4 shrink-0" />
              {email}
            </a>
          ) : null}
          {S.str(v.address) ? (
            <p className="flex gap-2 whitespace-pre-line leading-[var(--lp-lh-body)] opacity-90">
        ...
        <div className="space-y-2 text-sm">
          ...
          {links.map((l, i) => {
            ...
            return href ? (
              <a key={i} href={href} rel="noopener noreferrer nofollow ugc" className="flex min-h-11 items-center hover:underline">
                {S.str(l.label)}
              </a>
            ) : null;
          })}
      ...
      {S.str(v.copyright) ? (
        <div className="border-t border-white/10">
          <Container className="py-5 text-sm opacity-70">{S.str(v.copyright)}</Container>
        </div>
      ) : null}
    </footer>
  );
