...
/**
 * Listens for confirmx:calc-update events and toggles a `recommended`
 * class on the pricing card whose data-plan attribute matches the
 * calculator's current recommendation. No DOM is rendered — this
 * component is a side-effect-only listener mounted near the pricing
 * section.
 *
 * Mount order doesn't matter: if the calculator dispatched before we
 * subscribed, we read window.__confirmxCalc on mount to catch up.
 */
export function PricingHighlighter() {
  useEffect(() => {
    ...
    const w = window as unknown as { __confirmxCalc?: { plan?: string } };
    if (w.__confirmxCalc?.plan) applyHighlight(w.__confirmxCalc.plan);
