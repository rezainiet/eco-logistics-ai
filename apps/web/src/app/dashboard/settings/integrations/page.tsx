...
import { WebhookHealthCard } from "@/components/integrations/webhook-health-card";
...
export default function IntegrationsPage() {
  ...
  // First-run fork. Brand-new merchants — zero integrations, not in the
  // middle of an OAuth round-trip — get the focused <ConnectFlow /> UI
  // instead of the dense advanced grid. Once they connect anything (or
  // OAuth bounces them back here mid-flow with a callback param), the
  // existing advanced view takes over. Keeping the fork to a single
  // conditional block at the top of render means the existing 1,900+
  // lines of advanced UI are untouched and OAuth/webhook flow stays
  // exactly as before.
  const isFirstRun =
    !list.isLoading &&
  ...
  if (isFirstRun) {
    return (
      <div className="space-y-6">
        <ConnectFlow
          onSelectProvider={(provider) => setOpenProvider(provider)}
          shopifyConfigGap={
            entitlements.data?.platformShopifyConfigured === false
          }
        />
        {/* Render the connect dialog from the same render tree so
            selecting a provider opens the existing OAuth/credentials
            flow without a route change. The dialog component handles
            its own open/close state from `openProvider`. */}
        <ConnectDialog
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Webhook delivery summary — answers the "are events actually
          getting through?" trust question above the deeper system
          panel. Composes existing tRPC; renders nothing when there
          are no integrations. */}
      <WebhookHealthCard />
