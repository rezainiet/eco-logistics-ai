...
export default function BehaviorAnalyticsPage() {
  ...
  const overview = trpc.tracking.overview.useQuery({ days });
  const funnel = trpc.tracking.funnel.useQuery({ days });
  const top = trpc.tracking.topProducts.useQuery({ days, limit: 10 });
  const intent = trpc.tracking.highIntentSessions.useQuery({ days: Math.min(days, 14), limit: 15 });
  const repeat = trpc.tracking.repeatVisitors.useQuery({ days });
  const suspicious = trpc.tracking.suspiciousSessions.useQuery({ days: Math.min(days, 14), limit: 15 });
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        description="Storefront intent, abandon funnel, repeat visitors, and suspicious session detection — built on the embedded tracker."
        actions={
