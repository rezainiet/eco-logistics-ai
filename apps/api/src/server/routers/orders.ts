...
export const ordersRouter = router({
      ...
      if (mode === "replace") {
        ...
        for (const dup of duplicates) {
          ...
          if (updated) {
            ...
            await syncOrderInventory([updated._id as Types.ObjectId]);
  ...
  deleteOrder: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const merchantId = merchantObjectId(ctx);
      // Deleting an order that still holds stock gives the stock back first.
      const owned = await Order.exists({ _id: new Types.ObjectId(input.id), merchantId });
      if (owned) await reconcileOrderInventory(owned._id, { release: true });
  ...
  restoreOrder: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Re-reserve stock for a restored order (stock permitting).
      await syncOrderInventory([_id]);
  ...
  bulkRejectOrders: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (result.rejected.length > 0) {
        ...
        await syncOrderInventory(result.rejected);
      }
      void writeAudit({
        merchantId,
        actorId: merchantId,
