...
/**
 * Platform anomaly alert emailed to every admin (info / warning / critical).
 * Subject prefixes the severity so a mail-rule can shunt criticals to a
 * separate folder without parsing the body.
 */
export function buildAdminAlertEmail(args: {
  ...
  if (args.shortCount !== undefined) {
    detailRows.push(
      `<li><strong>Last hour:</strong> ${args.shortCount}</li>`,
    );
  }
  if (args.baselineRate !== undefined) {
    detailRows.push(
      `<li><strong>24h baseline:</strong> ${args.baselineRate.toFixed(1)}/h</li>`,
    );
  }
  const tone =
